...
//		Log.v("event.getAction()："+event.getAction()+"count："+event.getPointerCount(), "-------getX:"+event.getX()+"--------getY:"+event.getY());
	...
	{
        try {
//            Bitmap tmpBitmap = Bitmap.createBitmap(getWidth(), getHeight(), Config.ARGB_8888);//修改
			Bitmap tmpBitmap = Bitmap.createBitmap(getWidth(), getHeight(), Config.ARGB_4444);
//			Bitmap tmpBitmap = Bitmap.createBitmap(ScreenUtils.getScreenWidth(mContext), ScreenUtils.getScreenHeight(mContext), Config.RGB_565);

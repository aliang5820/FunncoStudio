    ...
    public void fillValues(final int position, View convertView) {
        ...
        if (getPositionForSection(serve.getTeam_id()) == position) {
            if (serve.getTeam_id().equals("0")) {
                ...
                holder.tvTip.setCompoundDrawablesWithIntrinsicBounds(null, null, null, null);
            } else {
                holder.tvTip.setText(serve.getTeam_name() + "");
                holder.tvTip.setCompoundDrawablesWithIntrinsicBounds(0, 0, R.mipmap.common_my_team, 0);
            }
        ...
        if (!TextUtils.isNull(weeks)) {
            ...
            for (String d : week) {
                ...
                if (!TextUtils.isNull(d)) {
                    ...
                    cb.setChecked(true);
                    cb.setTextColor(Color.parseColor("#fcfcfc"));
                }
            }
        }
        int hour = Integer.parseInt(serve.getDuration());
        holder.tvPrice.setText("耗时：" + DateUtils.getTime4Minutes2(hour) + "/" + serve.getPrice() + "RMB");

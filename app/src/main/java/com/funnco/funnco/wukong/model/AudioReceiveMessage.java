...
import com.funnco.funnco.wukong.viewholder.AudioReceiveViewHolder;
...
public class AudioReceiveMessage extends ReceiveMessage implements ItemClick.OnItemClickListener{
    ...
    private void initDrawable(Context context){
        if(mPalyDrawable == null) {
            Bitmap bmpPlay = BitmapFactory.decodeResource(context.getResources(), R.mipmap.btn_play_left);
            mPalyDrawable = new BitmapDrawable(bmpPlay);
            Bitmap bmpStop = BitmapFactory.decodeResource(context.getResources(),R.mipmap.btn_stop_left);
    ...
    private void displayAudioContent(final Context context,final AudioReceiveViewHolder viewHolder) {
        ...
        if(button != null){
            viewHolder.chatting_play_pause_btn.setBackgroundDrawable(mStopDrawable);

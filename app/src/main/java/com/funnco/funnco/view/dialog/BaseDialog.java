...
import com.funnco.funnco.view.textview.HandyTextView;
...
public class BaseDialog extends Dialog implements
	...
	public BaseDialog(Context context) {
		...
		mContext = context;
		setContentView(R.layout.layout_dialog_generic);
		initViews();
		initEvents();
		setCancelable(true);
	...
	public void setDialogContentView(int resource,boolean isGif) {
		View v = LayoutInflater.from(mContext).inflate(resource, null);
		if (mLayoutContent.getChildCount() > 0) {
			mLayoutContent.removeAllViews();
		}
//		if(isGif){
//			GifImageView iv = (GifImageView) v.findViewById(R.id.giv_dialog_loding);
//			MediaController mc = new MediaController(mContext);
//			mc.setMediaPlayer((GifDrawable) iv.getBackground());
//			mc.setAnchorView(iv);
//			mc.show();
//		}
		mLayoutContent.addView(v);

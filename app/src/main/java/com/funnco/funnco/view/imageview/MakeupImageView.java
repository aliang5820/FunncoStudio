...
public class MakeupImageView extends ImageView {
	...
	private void initViews(Context context, AttributeSet attrs) {
		...
		if(null == mLoading){
			mLoading = context.getApplicationContext().getResources().getDrawable(R.mipmap.chatting_default_image);
		}
		
		if(null == mForgroundDrawableLeft){
			mForgroundDrawableLeft = context.getApplicationContext().getResources().getDrawable(R.drawable.common_chat_arrow_left);
		}
		
		if(null == mForgroundDrawableRight){
			mForgroundDrawableRight = context.getApplicationContext().getResources().getDrawable(R.drawable.common_chat_arrow_right);
		}
...
//				int height = (int) a.getDimension(R.styleable.MakeupImageView_makeupHeight, DEFAULT_MAKEUP_HEIGHT);
				...
				if(height > 0) {
					height = FunncoUtils.px2dip(context, height);
				} else {

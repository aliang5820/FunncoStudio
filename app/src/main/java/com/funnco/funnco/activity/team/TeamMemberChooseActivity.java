...
public class TeamMemberChooseActivity extends BaseActivity {
    ...
    protected void initView() {
        ...
        fl.addView(expandableListView, 0);
        ...
        if (url.equals(FunncoUrls.getTeamMemberUrl())) {
            ...
            if (ls != null && ls.size() > 0) {
                ...
                if(isChat) {
                    ...
                    for(Team team:ls) {
                        List<TeamMember> members = team.getList();
                        List<TeamMember> newMembers = new ArrayList<>();
                        for(TeamMember member:members) {
                            if(!TextUtils.equals(member.getMember_uid(), user.getId())) {
                                newMembers.add(member);
                            }
                        }
                        team.setList(newMembers);
                        if(!newMembers.isEmpty()) {
                            newTeamList.add(team);
                        }
                    }
                    list.addAll(newTeamList);
                } else {
                    list.addAll(ls);
                }
            ...
            case R.id.llayout_foot://确定
                ...
                if (!TextUtils.isNull(team_id) && !TextUtils.isNull(team_name)) {
                    ...
                    if(memberList.size() == 1) {
                        //如果只是选择了一个团队成员,则返回它的头像
                        intent.putExtra("headpic", memberList.get(0).getHeadpic());
                    }
                }
                if (isChat) {
                    if (!isSingleChoice) {
    ...
    private String getIdsByMemberList() {
        ...
        for (TeamMember tm : memberList) {
            ...
            idArray += (tm.getMember_uid() + ",");
        }
        idArray = idArray.substring(0, idArray.length() - 1);
        LogUtils.e("------", "ids===" + idArray);
        return idArray;
    }

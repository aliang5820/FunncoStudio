 ...
 * with Alibaba.com.
 */
package com.funnco.funnco.activity.chat.impl;
...
import com.alibaba.wukong.im.MessageContent.MediaContent;
import com.alibaba.wukong.im.IMConstants.ErrorCode;
...
import com.funnco.funnco.impl.Cache;
import com.funnco.funnco.utils.PrefsUtil;
import com.funnco.funnco.utils.ThumbnailUtil;
...
//    }
    ...
    public void sendAlbumImage(final String url, final Conversation conversation, final boolean isNeedCompress) {
        if(TextUtils.isEmpty(url) || !FileUtils.isLocalUrl(url) || conversation == null) {
            return;
        }
            ...
            public void run() {
                ...
                message.sendTo(conversation,new Callback<Message>() {
                    ...
                    public void onException(String code, String reason) {
                        ...
                        if(isNeedCompress) {
                            String url = ((MediaContent) message.messageContent()).url();
...
//        ((Activity)context).startActivityForResult(intent,1);
    ...
    /**
     * 启动语音录音并发送语音。当语音消息发送失败时，语音文件将保存在应用数据目录。当该语音消息发送成功后， 调用{@link #(Cache)
    ...
    protected class AudioRecordListener implements OnRecordListener {
        ...
        public void onRecordStart(final String audioPath) {
            ...
            message.sendTo(mConversation,new Callback<Message>() {
                ...
                public void onException(String code, String reason) {
                    ...
                    if (url.startsWith(FILE_SCHEME) || ErrorCode.ERR_CODE_CANCELED.equals(code)) {
                        FileUtils.deleFile(audioPath);
                    } else {
        ...
        public void onRecordCompleted(String audioPath, List<Integer> volumns, long duration) {
            if(duration < 1000){
                ...
                FunncoUtils.showToast(mContext, "录音太短");
                return;
            }
            ...
            if (!FileUtils.hasSDcard()) {
                FileUtils.deleFile(audioPath);
                resetController();
                FunncoUtils.showToast(mContext, "SD卡不可用");
                return;
            }
        ...
        public void onRecordErrorListener(int i, String s) {
            resetController();
            FunncoUtils.showToast(mContext, "录音失败");
        }
    }
}

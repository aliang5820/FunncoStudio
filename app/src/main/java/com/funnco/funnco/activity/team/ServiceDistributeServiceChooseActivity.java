    ...
    protected void initView() {
        ...
        adapter = new CommonAdapter<Serve>(mContext,list,R.layout.layout_item_member_4) {
            ...
            public void convert(ViewHolder helper, Serve item, int position) {
                ...
                CheckableFrameLayout cf = helper.getView(R.id.id_layout_0);
...
//                    cf.toggle();
                    ((CheckBox)helper.getView(R.id.id_checkbox)).setChecked(true);
                }else{
                    idMap.remove(item.getId());
//                    cf.setChecked(false);
                    ((CheckBox)helper.getView(R.id.id_checkbox)).setChecked(false);
                }
            }
        };
